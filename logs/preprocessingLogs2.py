...
def nearestTeammate(px, py, teammates):
	...
	minDist = sqrt(pow(px - teammates[0][0], 2) + pow(teammates[0][1], 2))
	for i in range(1,3):
		dist = sqrt(pow(px - teammates[i][0], 2) + pow(teammates[i][1], 2))
		if (minDist > dist):
			minDist = dist
	...
	with open(sys.argv[1]) as file:
		for line in file:
			...
			if (line[i] == "GOAL"):
				...
				shotFile.write(line[i] + "\n")
				opponentFile.write("SHOOT\n")
			elif (line[i] == "UNSUCCESSFUL DRIBBLE"):
				dribbleFile.write(str(bx) + " " + str(by) + " ")
				opponentFile.write(str(bx) + " " + str(by) + " ")
				for j in range(0,3):
					dribbleFile.write(str(teammates[j]) + " ")
